...
module.exports.addRevenueValue = (req, res, next) => {
    ...
    Revenue.create({value,category, owner: userId})
    .then((value) => {
        if (!value){
            const error = new Error("Não foi possivel acrescentar o valor, verifique se é um número.");
            error.status = 400;
            next(error);
        }
        res.send({data:value})
...
module.exports.deleteRevenueValue = (req, res, next) => {
    ...
    .then((values) => {
        ...
        return Revenue.findByIdAndDelete(req.params._id)
        .then((deletedValue) => {
